...
check_explorer_setup() {
    ...
    # Check if the apps/explorer subdirectory exists
    if [ ! -d "$EXPLORER_DIR/apps/explorer" ]; then
        echo "❌ Explorer app directory not found: $EXPLORER_DIR/apps/explorer"
        echo "🔧 Need to install official explorer first"
        return 1
    fi
    ...
    if [ ! -f "package.json" ]; then
        echo "❌ No package.json found in explorer app directory"
        echo "🔧 Need to install official explorer first"
        return 1
    fi
